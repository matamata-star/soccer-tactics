...
/* ---------- 控え選手ベンチ帯（将棋の駒台のような、コート外の待機スペース） ---------- */
function renderBench() {
  ...
  const { W, H } = court();
  const y0 = H;
  el("rect", { x: -PAD, y: y0, width: W + PAD * 2, height: BENCH_DEPTH, fill: "rgba(8,12,18,0.55)" }, L);
  el("line", {
  ...
  const label = el("text", {
    x: 1, y: y0 + fs * 1.15, "font-size": fs, "font-weight": 700,
    fill: "rgba(255,255,255,0.55)",
  }, L);
  label.textContent = "ベンチ（控え）";
}
...
function buildTokens() {
  ...
  const { portrait, flip } = effectiveOrientation();
  // シーン全体の回転・反転を打ち消して、番号やボール柄が正しい向きで見えるようにする
  const counterRot = flip
...
function onPointerMove(e) {
  ...
  if (drag.mode === "token") {
    ...
    if (!p) return;
    const maxY = (drag.id !== "ball" && benchActive()) ? H + BENCH_DEPTH : H;
